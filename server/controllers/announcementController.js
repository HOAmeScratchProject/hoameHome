...
announcementController.getAllAnnouncements = async (req, res, next) => {
  try {
    const allAnnouncementsString = 'SELECT * FROM announcements ORDER BY datetime DESC';
    ...
    console.error('Error in announcementController.getAllAnnouncements.js: ', err);
    return next({
      log: `Error in announcementController.getAllAnnouncements.js ERROR:` + err,
       ...
       // const createAnnouncement = await db.query(createAnnouncement)
        ...
        if(!title || !message) {
            return next ({
              ...
              status: 400,
              message : { err : 'Title and message are required to create an announcement'},
              });
            }

        try {
          // query to create announcements
          const query = 'INSERT INTO announcements (title,message, datetime) VALUES ($1, $2, NOW()) RETURNING *';
          ...
          return next ({
            log: 'Error in announcementController.createAnnouncements: ERROR:', err,
            status: 400,
            message : { err : 'Title and message are required to create an announcement'},
            });
        }
